            ...
            try:
                ...
                try:
                    resolved = sync_source_sidecar(
                        cache,
                        iter_all_source_strings(self._modpack_path, self._cfg.language.code),
                        source_sidecar_path(cache_path),
                    )
                    self.log.emit(
                        f"已更新來源對照表 translation_sources.json（可反查 {resolved:,}/{len(cache):,}）。"
                    )
                except Exception as exc:  # noqa: BLE001 — 對照表失敗不可影響翻譯結果
                    self.log.emit(f"[警告] 來源對照表更新失敗（不影響翻譯）：{exc}")
