...
初始化完成後，Windows 會在專案資料夾建立版本化 launcher，例如 `模組包翻譯器v1.4.0.exe`。之後直接雙擊它即可啟動程式，不需要開終端機手動輸入命令。若 launcher 遺失，請重新執行 setup，或手動建立：
...
Windows 使用者也可以直接雙擊版本化 launcher，例如 `模組包翻譯器v1.4.0.exe`；它會先檢查是否已完成 setup，再在背景執行 `uv run python main.py`，launcher 錯誤會寫到 `.runtime/launcher.log`。
...
- Release ZIP 由 GitHub Actions 根據 `v1.4.0` 這類 tag 自動產生。
