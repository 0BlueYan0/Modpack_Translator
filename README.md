# Minecraft Modpack Translator v1.6.0
...
After setup, Windows builds a versioned launcher such as `模組包翻譯器v1.6.0.exe` in the project folder. Double-click it to start the app without opening a terminal. If the launcher is missing, run setup again or build it manually:
...
On Windows, users can also double-click the versioned launcher EXE, such as `模組包翻譯器v1.6.0.exe`. It checks that setup has been run, launches `uv run python main.py` in the background, and writes launcher errors to `.runtime/launcher.log`.

On startup, the app checks the latest GitHub Release in the background. If a newer release package is available, it shows an update dialog; if there is no update, it shows nothing. You can also click the **檢查更新** (Check for updates) button in the header at any time: when a new version exists you can update directly from the dialog (with download progress), and both "already up to date" and "check failed" are reported explicitly. The update downloads the release ZIP, verifies its SHA256 file when present, applies the new source files, removes the old `.venv` and stale local backend runtime files, runs setup again, and then restarts the app. Downloaded models (HuggingFace cache), the translation cache (`outputs/translation_cache.json`), and API settings are all preserved — no need to delete the folder and reinstall.
...
- Open the app. If a newer GitHub Release exists, click **直接更新** (Update now) in the update dialog. You can also check manually at any time via the **檢查更新** button in the header.
- The updater preserves user outputs (including the translation cache), downloaded models, and backups, but rebuilds `.venv` and the local backend setup to avoid dependency conflicts.
- Release ZIPs are generated by GitHub Actions from tags such as `v1.6.0`.
