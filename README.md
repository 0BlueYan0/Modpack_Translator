# Minecraft Modpack Translator v1.4.2
...
After setup, Windows builds a versioned launcher such as `模組包翻譯器v1.4.2.exe` in the project folder. Double-click it to start the app without opening a terminal. If the launcher is missing, run setup again or build it manually:
...
On Windows, users can also double-click the versioned launcher EXE, such as `模組包翻譯器v1.4.2.exe`. It checks that setup has been run, launches `uv run python main.py` in the background, and writes launcher errors to `.runtime/launcher.log`.
...
- Release ZIPs are generated by GitHub Actions from tags such as `v1.4.2`.
